...
import { getWorkouts, getSets, type SetRecord } from '@/lib/db.native';
...
const metrics = ['weight', 'reps'] as const;
...
function getTopSet(sets: SetRecord[]): SetRecord | undefined {
  return sets.reduce<SetRecord | undefined>((best, cur) => {
    const score = (s: SetRecord | undefined) =>
      s ? (s.weight ?? 0) * (1 + (s.reps ?? 0) / 30) : -Infinity;
...
export default function LastVsThis() {
  ...
  useEffect(() => {
    let canceled = false;
    (async () => {
      const workouts = await getWorkouts(exercise); // newest first (DESC)
      const pstToday = todayPstStr();

      const todayW = workouts.find(w => w.date === pstToday) ?? null;
      const lastW  = workouts.find(w => w.date < pstToday)  ?? null;
      ...
      setTodayTop(todayW ? getTopSet(await getSets(todayW.id)) : undefined);
      setLastTop( lastW  ? getTopSet(await getSets(lastW.id))  : undefined);
    })();
  ...
  /* toast PR */
  useEffect(() => {
    if (lastTop && todayTop && (todayTop.weight ?? 0) > (lastTop.weight ?? 0)) {
  ...
  const rows = useMemo(() => {
    const vol = (s?: SetRecord | null) =>
      s?.weight != null && s.reps != null ? s.weight * s.reps : null;
    const build = (metric: 'weight' | 'reps' | 'volume', prev: number | null | undefined, cur: number | null | undefined) => {
      const delta = prev != null && cur != null ? cur - prev : null;
      return { metric, prev, cur, delta };
    };
    return [
      build('weight', lastTop?.weight ?? null, todayTop?.weight ?? null),
  ...
  const todayLabel = todayDate && todayDate === todayPstStr()
    ? `Today (${todayDate})`
    : todayDate ?? 'Today';
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: theme.spacing(4), gap: theme.spacing(4), backgroundColor: theme.colors.bg }}>
      ...
      <Card style={{ padding: theme.spacing(2) }}>
        {shown.map(ex => (
          <Pressable key={ex} onPress={() => setExercise(ex)} style={[styles.chip, ex === exercise && styles.chipActive]}>
            <Text style={{ color: theme.colors.text }}>{ex}</Text>
          </Pressable>
        ))}
        {shown.length === 0 && <Text style={{ color: theme.colors.text }}>No match 🤷‍♂️</Text>}
      </Card>
        ...
        {rows.map(({ metric, prev, cur, delta }) =>
          prev == null && cur == null ? null : (
            <View key={metric} style={styles.row}>
              ...
              <Text style={[styles.deltaCol, styles.text, { color: delta != null && delta > 0 ? theme.colors.accent : theme.colors.text }]}>{delta == null ? '-' : delta > 0 ? `+${delta}` : delta}</Text>
              <Text style={[styles.valCol,   styles.text]}>{cur   ?? '-'}</Text>
            </View>
          )
        )}

        <Text style={styles.footnote}>Δ compares today’s top-set vs. previous workout’s top-set (dates in PST/PDT).</Text>
      </Card>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  search: { color: theme.colors.text, borderWidth: 1, borderColor: theme.colors.muted, padding: theme.spacing(2), borderRadius: 6 },
  chip:   { marginRight: theme.spacing(2), marginBottom: theme.spacing(2), paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: theme.colors.muted, borderRadius: 6 },
  chipActive: { borderColor: theme.colors.accent },
  headline:   { fontSize: 20, fontFamily: theme.fonts.headline, color: theme.colors.text, marginBottom: theme.spacing(2) },
  row:        { flexDirection: 'row', alignItems: 'center', marginBottom: theme.spacing(1) },
  metricCol:  { flex: 1 },
  ...
  footnote:   { marginTop: theme.spacing(2), color: theme.colors.muted, fontSize: 12, textAlign: 'center' },
});
