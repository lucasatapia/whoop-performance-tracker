// app/(tabs)/progress.tsx — Progress Tab (top‑set 1‑RM trend)
...
export default function ProgressTab() {
  ...
  useEffect(() => {
    let canceled = false;
    (async () => {
      const token = await storage.getItem("token");   // ⬅ storage wrapper
      if (!token) return;

      const workouts: WorkoutRow[] = await getWorkouts(token);
      const end   = new Date();
      const start = new Date(end);
      start.setDate(end.getDate() - periodDays);

      // filter workouts in range and compute daily top‑set est‑1RM
      const hist = workouts
        .filter((w) => {
        ...
        .map((w) => {
          const top = w.sets
            ...
            .reduce<SetRecord | undefined>((best, cur) => {
              const score = (s?: SetRecord) =>
                s ? (s.weight_kg ?? 0) * (1 + (s.reps ?? 0) / 30) : -Infinity;
              return score(cur) > score(best) ? cur : best;
            }, undefined);

          return top
            ? { date: w.date, topWeight: top.weight_kg ?? 0, reps: top.reps ?? 0 }
            : null;
        })
        .filter(Boolean) as { date: string; topWeight: number; reps: number }[];
  ...
  const { tickValues, tickFormat } = useMemo(() => {
    ...
    return {
      tickValues: months,
      tickFormat: (t: Date) => t.toLocaleDateString("en-US", { month: "short" }),
    };
  }, [periodDays]);
        ...
        <View style={styles.buttonRow}>
          {TIMEFRAMES.map(({ label, days }) => (
            <Pressable
              ...
              style={[styles.button, periodDays === days && styles.buttonActive]}
            >
              <Text
                style={[
                  styles.buttonText,
